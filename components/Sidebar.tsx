...
const NAV = [
  ...
  { href: "/reports", label: "DMARC-Reports", icon: Mail, admin: false },
  { href: "/sources", label: "Sender-Quellen", icon: Server, admin: false },
  { href: "/security", label: "Mail-Security", icon: ShieldCheck, admin: false },
  { href: "/alarms", label: "Alarme", icon: Bell, admin: false },
  ...
  return (
    <aside className="relative z-10 flex w-60 shrink-0 flex-col border-r border-zinc-800/70 bg-zinc-950/80 backdrop-blur">
      <div className="flex h-16 shrink-0 items-center gap-3 border-b border-zinc-800/70 px-5">
        <Logo size={32} />
        <div>
          <p className="text-xs font-semibold tracking-tight text-zinc-100">NexDMARC</p>
          <p className="text-[10px] text-muted-foreground">Mail Security</p>
        </div>
      </div>
